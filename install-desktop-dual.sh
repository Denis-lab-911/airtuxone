#!/usr/bin/env bash
# Installe un raccourci bureau / menu pour lancer AirTux One Dual
set -euo pipefail

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LAUNCHER="$PROJECT_ROOT/airtuxone-dual.sh"
ICON="$PROJECT_ROOT/airtuxone_logo.png"
DESKTOP_DIR="$(xdg-user-dir DESKTOP 2>/dev/null || echo "${XDG_DESKTOP_DIR:-$HOME/Desktop}")"
APPS_DIR="$HOME/.local/share/applications"
DESKTOP_FILE="airtuxone-dual.desktop"
...
Version=1.0
Type=Application
Name=AirTux One Dual
Comment=VelocityOne Flightstick → 2 Xbox pads virtuelles (MSFS / GeForce NOW, profil dual)
Exec=bash -c '"$LAUNCHER" || { echo; read -p "Erreur — Entrée pour fermer..." _; }'
Path=$PROJECT_ROOT
Icon=$ICON
Terminal=true
Categories=Game;Utility;
Keywords=flight;simulator;gamepad;xbox;velocityone;dual;
StartupNotify=true
EOF
    chmod +x "$dest"
}
...
echo "[OK]  Raccourci Dual installé :"
echo "      Bureau    → $DESKTOP_DIR/$DESKTOP_FILE"
echo "      Menu apps → $APPS_DIR/$DESKTOP_FILE"
echo ""
echo "Double-cliquez sur l’icône pour lancer le mode 2 manettes."
